...
Suno는 추상적이거나 문학적인 표현("슬픔이 밀려오는 느낌")보다, 실제로 들리는 소리를 구체적인 프로덕션/오디오 엔지니어링 용어로 지시할 때("sparse piano notes, long reverb tail, minor key sustain") 훨씬 더 잘 알아듣고 반영해. 그러니 의미는 그대로 유지하면서:
(1) 같은 단어·구절을 여러 섹션에서 반복하지 말고 매번 다른 표현으로 다양화하고
(2) 악기·이펙트·다이나믹·공간감처럼 실제로 소리로 구현되는 구체적 프로덕션 용어로 디테일을 더해줘 — Suno가 못 알아들을 모호하거나 시적인 비유로 흐르면 안 돼
