...
나머지는 악기/편곡/구조/믹스/보컬/무드/전개 중 지금 조합에 실제로 도움될 관점으로 2~4개 더 채워줘 (뻔한 일반론 금지). 모든 카테고리에서 "전문 음악 프로듀서가 실제로 트랙을 검토하듯" 다양한 각도로 봐 — 표면적인 칭찬이나 뻔한 조언 말고, 실제 텍스트에 근거한 구체적 지적이어야 해. 아래는 반드시 실제 텍스트에서 확인해서, 문제가 있으면 해당 카테고리에 포함시켜:
...
- (Anti-AI 필터 ON일 때만) 지금 텍스트가 AI가 만든 전형적인 음악처럼 뻔하고 기계적으로 들릴 위험이 있는지 확인해 — [현재 설정]에 이미 "organic, warm, human-feel, analog imperfections, natural dynamics" 같은 범용 태그가 항상 붙어있는데, 이것만으로는 부족해. 이 트랙 고유의 구체적인 "의도적 불완전함"이나 사람이 연주하는 듯한 디테일(예: 특정 구간에서 타이밍이 살짝 밀리는 느낌, 벨로시티가 균일하지 않은 느낌, 필터가 완벽히 대칭적이지 않은 느낌)을 tag나 boostText로 더 채워 — 뻔한 "organic" 반복 말고 이 곡만의 구체적인 인간적 디테일이어야 해
