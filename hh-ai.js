...
function buildWriteSpec(prev){
  ...
  return {
    ...
    brief:effectiveBrief()?{analysisBasis:'title-based model knowledge, not audio-verified',analysisEvidence:st.brief.analysisEvidence||null,cueBasis:st.brief.cueBasis||{},uncertainFields:st.brief.uncertainFields||[],instrumentalProfile:st.brief.instrumentalProfile||{},understood:st.brief.understood,styleTags:effectiveBrief().styleTags||[],cues:effectiveBrief().cues||{}}:null,
...
[섹션 디렉팅]
- 각 섹션을 작성한 뒤 스타일과 이전 구간에 이미 있는 설명을 삭제해. 같은 그루브를 유지하는 구간은 유지 문장만으로 충분하다. 악기별로 한 문장씩 채우지 마. 작은 필인·음역 상승·패드 확장·정해진 마디마다의 장식을 차별화 목적으로 만들어내지 마. 구간 역할을 바꾸는 꼭 필요한 변화만 남겨. 예: (Return to the established groove; keep the same restrained energy.)처럼 짧아도 완성된 구간이다. 이 예시 문구를 모든 구간에 복사하지 마.
- 스타일의 기본 편성·음색·그루브는 따로 재서술하지 않아도 유지된다. 구간마다 드럼·베이스·멜로디·공간을 모두 설명하는 체크리스트를 만들지 마. 변화가 없는 훅은 기존 그루브 유지처럼 짧게 써도 된다. 지시 개수는 고정하지 마.
...
const TYPE_BEAT_WRITE_STATIC=`너는 레퍼런스 반주 기반 타입비트를 설계하는 프로듀서다. 원곡과 다른 곡을 쓰되 같은 스타일 범위를 유지해. 원곡의 정확한 편곡 재현은 목표가 아니다.
${STYLE_BUDGET_GUIDE}
${REFERENCE_DEVELOPMENT_GUIDE}
[타입비트 스타일 작성]
- 먼저 제공된 설계에서 곡을 구별하는 핵심 관계를 정해: 무드가 어떤 리듬·음색·여백에서 생기는가, 무엇이 그루브를 이끌고 무엇이 받치는가. 스타일의 중심 문장으로 그 관계를 설명해. 악기 목록이나 모든 분석 필드의 요약문을 쓰는 것이 목표가 아니다.
- 예를 들어 리듬 중심 설계라면 킥·베이스·타악의 맞물림과 멜로디가 비켜주는 공간을, 질감 중심 설계라면 음의 길이·거리와 반복 패턴의 관계를 설명해. 어떤 곡에도 특정 악기나 주법을 일괄 적용하지 마. 멜로디가 정체성인 설계라면 새 모티프를 허용하되 분석에서 뒷받침한 역할 범위를 유지해.
- Hook은 구조상의 구간 이름이지 기타/신스 솔로 훅을 만들라는 뜻이 아니다. sing-along chorus 같은 보컬 중심 설명을 무보컬로 옮길 때는 반주의 리듬·여백·에너지만 활용하고, 보컬의 기억하기 쉬운 선율을 반주 악기의 의무로 넘기지 마.
- 악기별 지시를 보태기 전에 그 디테일이 핵심 분위기를 어떻게 더 잘 구현하는지 판단해. 이유 없는 패드 확장·옥타브 응답·고음 변주·필인은 생략해. 이미 맞는 훅은 그대로 돌아오는 것만으로 충분하다. 효과적인 디테일은 남기되 지시 개수나 문장 수를 고정하지 마.
- 사용자 지정 BPM/Key·보컬·악기·제외 조건은 지켜. 실제 아티스트·곡 이름은 출력하지 마. 제목을 보고 새 분석을 시작하지 말고 제공된 설계만 사용해.
- 새로 기억할 훅·상승 하강 음형·악기 간 대화를 의무적으로 만들지 마. 목표 스타일에 어울리는 새 멜로디·코드·필인·세부 전개를 설계해도 돼. 새로운 아이디어를 원곡의 사실로 말하지 말고, 무드·바운스·음색·전체 밀도·에너지 범위를 바꾸지 마.
- 설계서 sound.balance는 무엇이 앞에 있고 뒤에 있는지, sound.activity는 얼마나 자주 연주하는지, sound.timbreSpace는 밝기·어택·잔향·거리, sound.vocalSpace는 보컬을 위한 여백이다. 이 정보를 스타일의 악기 역할 문장에 반영해. 원곡 악기 비중을 정확히 복제할 필요는 없지만, 보조 파트를 과장해 비트를 솔로 연주곡으로 바꾸지 마.
- unknownBalanceFields에 있는 비중·활동량·음색·여백은 판단 불가다. sectionCues의 catchy hook 같은 요약만으로 지속적인 전면 리드나 정확한 응답 빈도·음역·솔로 비중을 만들어내지 마. 정보가 없어도 같은 스타일 안의 새로운 반주 역할은 설계할 수 있어. 이를 원곡에서 확인된 역할이라고 주장하지 마.
- lead/background 메뉴 이름은 분석된 비중보다 우선하지 않는다. 사용자가 직접 악기 역할을 바꿨으면 그 변경만 반영해. 분석이 불확실하면 임의로 기타를 크게 하거나 모든 악기를 작게 하지 말고, 목표 스타일에 맞는 역할을 과장 없이 설계해.
...
${WRITE_STATIC.slice(WRITE_STATIC.indexOf('[섹션 디렉팅]'),WRITE_STATIC.indexOf('[선택값과 레퍼런스]'))}
${WRITE_STATIC.slice(WRITE_STATIC.indexOf('[무보컬과 보컬 — 사용자 선택 최우선]'))}
...
- roleEvidenceMissing은 원곡의 비중을 모른다는 뜻이지 리드 악기를 금지하라는 뜻이 아니다. 주어진 무드·그루브·편성에 가장 어울리는 역할과 주법을 설계하고 선택 이유를 musicPlan에 남겨. 기타 중심·베이스 중심·질감 중심 중 어느 것도 기본 정답이 아니다. 새 역할을 원곡에서 확인된 사실로 주장하지 말고 사용자 지정 조건은 우선해.
...
Keep audible actions and relationships, not a tag list. Remove repeated adjectives, instrument-by-instrument elaboration and section-by-section narration first. Section context is for consistency only: do not copy it into the style. Maintain the original energy range and do not turn backing parts into solo leads. Preserve emotional darkness/brightness and perceived groove independently of numeric BPM/key; never replace a relaxed or mid-tempo feel with high-tempo energy just because the BPM number is large. Keep explicit user constraints over generic defaults. Do not cut a sentence midway. Return JSON {"essential":"A complete concise paragraph carrying ALL indispensable identity and explicit constraints, under 700 characters", "optional":["Complete optional sentence."]}. Optional sentences must contain only expendable elaboration, never BPM/key, vocal choice, core mood/groove or user requirements. The app may omit optional sentences to fit the actual character limit. Do not repeat the essential paragraph in optional sentences.`;
...
function typeBeatPlan(spec){
  const brief=filterReferenceUncertainty({kind:'song',...spec.brief});
...
For every instrument decide why it suits THIS intention, what role it plays, how it is performed, and where that performance helps. Distinguish supplied reference information from your new compositional choices; do not claim new choices were heard in the original. Decide the defining relationship among rhythm, bass, melody and space. Every part must have a purpose; no obligatory pad, octave doubling, fills or final climax. Section changes must serve that relationship. Returning to the established arrangement is a complete decision. Choose section-specific role changes when they serve the musical intention, explain them in the design, and preserve the reference mood and groove. Do not assume that quieter, sparser or less melodic is better. A prominent guitar, bass hook, rich arrangement or solo passage is appropriate when supported by the intention; no instrument is inherently preferred or discouraged. Do not infer brightness or perceived speed from key/BPM.
...
Return JSON only: {"identity":"mood, perceived groove, energy range and defining sound relationship", "roles":[{"part":"instrument or rhythm part", "function":"musical purpose, prominence, activity and space", "performance":"articulation, rhythm or phrasing suited to this part", "fitReason":"why this choice fits the supplied intention; distinguish reference clues from creative choices"}], "sections":[{"header":"exact supplied header", "direction":"only the necessary change from the base arrangement, or maintain it"}]}. Use English sound directions. Include every supplied header exactly once in order. Do not add instruments merely to give each section something new.`;
async function buildMusicPlan({mode,spec,prev}){
  ...
  return {identity:plan.identity,roles:plan.roles.map(({part,function:purpose,performance,fitReason})=>({part,function:purpose,...(typeof performance==='string'?{performance}:{}),...(typeof fitReason==='string'?{fitReason}:{})})),sections:plan.sections.map(({header,direction})=>({header,direction}))};
}
const MUSIC_PLAN_RENDER_GUIDE=`
[공통 음악 설계의 표현 단계]
아래 musicPlan은 이번 곡의 공통 음악적 설계다. 스타일은 identity와 roles를 자연스러운 한 문단으로 표현하고, 섹션은 동일한 역할 위에서 sections.direction의 변화만 표현해. 두 출력에서 별도로 작곡하지 마. 계획에 있는 구간별 역할 변화와 주법은 살려 표현하되, 계획에 없는 역할 반전이나 레이어·필인을 임의로 보태지 마. fitReason은 선택 이유를 이해하는 참고이며 최종 프롬프트에는 설명문 대신 실제 소리·연주 지시로 표현해. 설계의 반복·유지 지시도 완성된 편곡이다. 사용자의 명시적 조건과 충돌하면 사용자 조건을 우선해. 실제 가사 보존 및 출력 형식은 기존 계약을 지켜.`;
...
function effectiveBrief(){return st.brief?filterReferenceUncertainty(st.brief):null;}
...
function buildBriefProposal(text,p){
  ...
  return {text,kind:p.kind==='song'?'song':'vibe',understood:String(p.understood||'').slice(0,300),reason:String(p.reason||'').slice(0,200),v,styleTags,cues,instrumentalProfile,analysisEvidence:p.analysisEvidence||null,cueBasis:p.cueBasis||{},uncertainFields:p.uncertainFields||[],items};
}
...
function applyBrief(opts={}){
  ...
  st.brief=on('sound')?{text:P.text,kind:P.kind,understood:P.understood,styleTags:P.styleTags,cues:P.cues,source:P.source||'ai',instrumentalProfile:P.instrumentalProfile||{},analysisEvidence:P.analysisEvidence||null,cueBasis:P.cueBasis||{},uncertainFields:P.uncertainFields||[]}:null;
...
function briefAnalysisDetails(brief){
  ...
  const rows=Object.entries(labels).map(([key,label])=>{
    ...
    const basis=brief.kind!=='song'?'새 곡 설계 제안':e?.basis==='model-knowledge'?'모델 지식 · 음원 미검증':e?.basis==='inference'?'추정 · 작성에서 제외':e?.basis==='unknown'?'판단 불가 · 작성에서 제외':'근거 미기록 · 음원 미검증';
    return '<dt>'+label+'</dt><dd>'+escHtml(brief.instrumentalProfile?.[key]||'분석 정보 없음 — 임의로 확정하지 않음')+'<br><small>'+basis+(e?.reason?' — '+escHtml(e.reason):'')+'</small></dd>';
  }).join('');
